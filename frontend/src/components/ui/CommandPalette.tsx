...
import { Search, Command, ArrowRight, Monitor, Package, CreditCard, LayoutDashboard, Ticket, Users, Plus, Sparkles } from 'lucide-react';
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
    ...
    const items = [
        ...
        // Actions (Role based)
        ...(can('tickets.create') ? [{ id: 'action-ticket', type: 'Action', name: 'Create Ticket', icon: Plus, action: () => { navigate('/maintenance'); /* Ideally open modal via store state */ } }] : []),
        ...(can('assets.create') ? [{ id: 'action-asset', type: 'Action', name: 'Add Asset', icon: Plus, action: () => { navigate('/assets'); } }] : []),
        ...(user?.role === 'SUPER_ADMIN' || user?.role === 'ADMIN' ? [
            { id: 'nav-users', type: 'Navigation', name: 'Manage Users', icon: Users, action: () => navigate('/admin/users') }
        ] : []),
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh] sm:pt-[25vh]">
            ...
            <div 
                className="relative w-full max-w-xl bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl shadow-2xl overflow-hidden mx-4 pb-2 animate-in fade-in zoom-in-95 duration-200"
                role="dialog"
                aria-modal="true"
            >
                {/* Header / Search */}
                ...
                <div className="max-h-[60vh] overflow-y-auto p-2">
                    {filteredItems.length === 0 ? (
                        <div className="py-12 px-4 text-center text-[var(--text-secondary)]">
                            ...
                            <p className="text-sm mt-1 opacity-70">Try searching for a page or action.</p>
                        </div>
                    ) : (
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <Icon className={`w-5 h-5 ${item.color || (isSelected ? 'text-[var(--primary)]' : 'text-[var(--text-secondary)]')}`} />
