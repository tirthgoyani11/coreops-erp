          ...
          <SidebarMenu>
            {support.map((item) => (
              <SidebarMenuItem key={item.name}>
                <SidebarMenuButton tooltip={item.name} isActive={pathname === item.url} className="text-neutral-500 dark:text-neutral-400 text-xs h-8 font-medium data-[active=true]:border data-[active=true]:border-border data-[active=true]:text-neutral-600 dark:data-[active=true]:text-neutral-300 hover:border hover:border-border hover:text-neutral-600 dark:hover:text-neutral-300" render={<a href={item.isDisabled ? "#" : item.url} onClick={(e) => {
                                        if (item.isDisabled) {
                                          e.preventDefault();
                                        }
                                      }} />}><item.icon className="" /><span className="tracking-tight">{item.name}</span></SidebarMenuButton>
              </SidebarMenuItem>
            ))}
          </SidebarMenu>
        </CollapsibleContent>
