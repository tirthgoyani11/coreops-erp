          ...
          <SidebarMenu>
            {apps.map((app) => (
              <SidebarMenuItem key={app.name}>
                <SidebarMenuButton tooltip={app.name} className="text-neutral-500 dark:text-neutral-400 text-xs h-8 font-medium data-[active=true]:border-border hover:border hover:border-border data-[active=true]:text-neutral-600 dark:data-[active=true]:text-neutral-300 hover:text-neutral-600 dark:hover:text-neutral-300" render={<a href={app.isDisabled ? "#" : app.url} onClick={(e) => {
                                        if (app.isDisabled) {
                                          e.preventDefault();
                                        }
                                      }} />}><app.icon /><span className="tracking-tight">{app.name}</span></SidebarMenuButton>
              </SidebarMenuItem>
            ))}
          </SidebarMenu>
        </CollapsibleContent>
